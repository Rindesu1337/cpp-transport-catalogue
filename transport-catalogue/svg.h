...
struct Rgb {
    ...
    Rgb(uint8_t re, uint8_t gre, uint8_t blu) 
    : red(re)
    , green(gre)
    , blue(blu) {
    }
...
struct Rgba {
    ...
    Rgba(uint8_t re, uint8_t gre, uint8_t blu, double opaci)
    : red(re)
    , green(gre)
    , blue(blu)
    , opacity(opaci) {
...
struct RenderContext {
    ...
    RenderContext(std::ostream& output, int indent_steps, int indents = 0)
        : out(output)
        , indent_step(indent_steps)
        , indent(indents) {
    }
...
protected:
    ...
    void RenderAttrs(std::ostream& out) const {
        ...
        if (fill_color_) {
            ...
            std::visit(ColorPrint{strm}, *fill_color_);
            out << "fill=\""sv << strm.str() << "\""sv;
        }
        if (stroke_color_) {
            std::ostringstream strm;
            std::visit(ColorPrint{strm}, *stroke_color_);
