...
void input_reader::InputReader::ParseLine(std::string_view line) {
    auto command_description = input_reader::detail::ParseCommandDescription(line);
    if (command_description) {
        commands_.push_back(std::move(command_description));
    }
}

void input_reader::InputReader::ApplyCommands([[maybe_unused]] transport::TransportCatalogue& catalogue) const {
    ...
    for (auto& command_: commands_) {
        if (command_.command == "Bus") {
            catalogue.AddBus(input_reader::detail::Trim(command_.id), input_reader::detail::ParseRoute(command_.description));
        }
    }
}